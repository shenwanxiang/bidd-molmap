...
def get_deepchem_idx(df):
    """ deepchem dataset"""
    deepchem_data_name = './FreeSolv_deepchem.data'
    if os.path.exists(deepchem_data_name):
        train_df,valid_df,test_df = load(deepchem_data_name)
    else:
        import deepchem as dc
        task, train_valid_test, _ = dc.molnet.load_sampl(featurizer='Raw',split = 'random')
...
def get_attentiveFP_idx(df):
    """ attentiveFP dataset"""
    train, valid,test = load('./FreeSolv_attentiveFP.data')
