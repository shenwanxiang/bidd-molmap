...
def DoublePathClassificationModel(molmap1_size, molmap2_size, n_outputs = 1):
    ...
    d1 = Dense(128,activation='tanh')(merge)
    d1 = Dense(64,activation='tanh')(d1)
    outputs = Dense(n_outputs, activation='linear')(d1)
...
if __name__ == '__main__':
    ...
    for mtype in model_types:
        ...
        for seed in [7, 77, 777]:
            ...
            etc_perf = performace.evaluate(etcX, etcY)            
            ...
            best_etc_pred_y = list(performace.model.predict(etcX).reshape(-1,))
            ...
            final_res = {'train_rmse':np.nanmean(train_perf[0]), 
                         ...
                         'etc_rmse':np.nanmean(etc_perf[0]),
        ...
        pd.DataFrame(results).to_csv('../results/results_05_MolMapNet_%s.csv' % mtype)
