...
mp1_new =  molmap.loadmap('../descriptor.mp')
mp1_new.fit(method = 'umap', min_dist = 0.6, n_neighbors = 20)    
    
mp2_new =  molmap.loadmap('../fingerprint.mp')
mp2_new.fit(method = 'umap', min_dist = 0.6, n_neighbors = 20)
...
X1 = mp1.rearrangement(X1, mp1_new)
X2 = mp2.rearrangement(X2, mp2_new)
...
pd.DataFrame(results).to_csv('./%s.csv' % task_name)
